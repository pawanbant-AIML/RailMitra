...
class BookingService:
    ...
    def create_mock_booking(self, entities: dict, db: Session) -> schemas.Booking:
        """
        Create a booking from an entities dict.

        Accepts BOTH the old key names used by NLPService
            ("class_type", "passenger_count")
        AND the new key names used by ChatNLPService / chat.py
            ("travel_class", "passengers")
        so both callers work without changes on their side.
        """
        ...
        # ── Travel class  (accept both key spellings) ──────────────────────
        travel_class = (
            entities.get("travel_class")          # new key (ChatNLPService)
            or entities.get("class_type")         # old key (NLPService)
            or "SL"
        )

        # ── Passenger count  (accept both key spellings) ───────────────────
        raw_pax = (
            entities.get("passengers")            # new key  (int or str)
            or entities.get("passenger_count")    # old key  (str)
            or "1"
        )
        WORD_TO_NUM = {
        ...
        pax_str = str(raw_pax).strip().lower()
        passenger_num = (
            int(pax_str)
            if pax_str.isdigit()
            else WORD_TO_NUM.get(pax_str, 1)
        )

        # ── Find a real train for this route ───────────────────────────────
        trains       = self.search_trains(entities, db)
        train_number = trains[0].train_number if trains else entities.get("train_number", "00000")
