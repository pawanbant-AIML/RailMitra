...
def chat_endpoint(messages: List[schemas.ChatMessage], db: Session = Depends(get_db)):
        ...
        if not src or not dst:
            ...
            try:
                booking_entities = {
                    ...
                    "destination_station": dst,
                    "date": analysis.entities.date,
                    # Use "travel_class" / "passengers" – the patched
                    # booking_service.create_mock_booking() accepts both spellings.
    ...
    # ── CANCEL BOOKING ──────────────────────────────────────────────────────
    elif analysis.next_action == "CANCEL_BOOKING":          # was: analysis.intent
        ...
        if not bid:
            ...
            if success:
                ...
                reply = (
                    ...
                    'Say *"Show my bookings"* to check your valid booking IDs.'
                )

    # ── BOOKING HISTORY ─────────────────────────────────────────────────────
    elif analysis.next_action == "BOOKING_HISTORY":         # was: analysis.intent
    ...
    # ── CHECK ROUTE ─────────────────────────────────────────────────────────
    elif analysis.next_action == "CHECK_ROUTE":             # was: analysis.intent
        ...
        if tn:
            ...
            if stops:
                ...
                for s in stops[:20]:
                    ...
                    lines.append(
                        f"  {s.sequence:>2}. {s.station_code:<8}  "
                        f"arr:{arr}  dep:{dep}"
                    )
                if len(stops) > 20:
